...
db.restaurants.find({"cuisine":{"$ne":"American"},"grades.grade":{"$eq":"A"}, "borough":{"$ne":"Brooklyn"}}, { "_id": 0 }).sort({"cuisine": -1})
...
db.restaurants.find({"$or": [{ "$and": [ { "cuisine": "Seafood" }, { "cuisine": { "$nin": ["American", "Chinese"] } } ] },{ "name": /^Wil/ }]}, { "_id": 0 })
...
db.restaurants.find({"location.coordinates" : { $type: "double" }}, {"name":1, "restaurant_id":1, "coordinades":1,"_id" : 0})
